...
/**
 * Robolectric has no AndroidKeyStore provider (it's a platform HAL). Any
 * composable whose init path touches AuthManager/SecureBlobStore hits
 * KeyStore.getInstance("AndroidKeyStore") → NoSuchAlgorithmException.
 * Register a delegating fake provider once per JVM.
 */
object FakeAndroidKeyStore {
    val setup by lazy {
        Security.addProvider(
            object : Provider("AndroidKeyStore", 1.0, "Robolectric fake") {
                init {
    ...
    class FakeKeyStore : KeyStoreSpi() {
        ...
        override fun engineGetCreationDate(alias: String?): java.util.Date = wrapped.getCreationDate(alias)
    ...
    class FakeAesKeyGenerator : KeyGeneratorSpi() {
        ...
        override fun engineInit(params: java.security.spec.AlgorithmParameterSpec?, random: SecureRandom?) = Unit
